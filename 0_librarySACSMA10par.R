...
if (Testmodel_ind == 3){ # SacSma
  ...
  Range <- matrix(data = c(5,300,5,150,10,700,100,1200,
                           ...
                           0,0.4),nrow = 2,ncol = d,byrow = FALSE)
  known_pars<- c(0.3,0,0)
  
}


if (Testmodel_ind == 3.1){ # SacSma with only 10 parameters
  source("sacSma.R")
  load("SacSma_dataframe")
  load("SACSMA_pars_fix")
  ...
  Range <- matrix(data = c(5,300,5,150,10,700,100,1200,
                           5,500,0.1,0.75,0.001,0.05,0.01,0.6,
                           5,350,1,5,0,0.9,0,0.1,
                           0,0.4),nrow = 2,ncol = d+3,byrow = FALSE)
  fix_vals<- colMeans(Range[,pars_fix])
  
  Range<- Range[,-pars_fix]
  known_pars<- c(0.3,0,0)
}
