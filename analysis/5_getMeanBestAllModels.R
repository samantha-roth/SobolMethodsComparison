#is the mean best across all seeds for each model the same? 
...
load("./Ranking_Data/textMat_MeanBest")
text_Gfunc<- textMat_MeanBest; rm(textMat_MeanBest)

load("./polynomial/Ranking_Data/textMat_MeanBest")
text_poly<- textMat_MeanBest; rm(textMat_MeanBest)

folder<- paste0(folderpath,"Hymod")
load(paste0(folder,"/textMat_MeanBest"))
text_Hymod<- textMat_MeanBest; rm(textMat_MeanBest)

folder<- paste0(folderpath,"SacSma10")
load(paste0(folder,"/textMat_MeanBest"))
text_SACSMA10<- textMat_MeanBest; rm(textMat_MeanBest)

text_meanBestAllMod<- matrix(NA,nrow=nrow(text_poly),ncol=ncol(text_poly))

for(i in 1:nrow(text_Gfunc)){
  for(j in 1:ncol(text_Gfunc)){
    if(!is.na(text_Gfunc[i,j]) & !is.na(text_poly[i,j])){
      if(text_Gfunc[i,j]==text_poly[i,j]) text_meanBestAllMod[i,j]<- text_Gfunc[i,j]
    }
  }
}
...
for(j in 1:ncol(text_Hymod)){
  if(!is.na(text_meanBestAllMod[i,j])){
    if(is.na(text_Hymod[1,j])){
      text_meanBestAllMod[i,j]<- NA
    } else if (text_Hymod[1,j]!=text_meanBestAllMod[i,j]){
      text_meanBestAllMod[i,j]<- NA
    } 
  }
}
...
for(j in 1:ncol(text_SACSMA10)){
  if(!is.na(text_meanBestAllMod[i,j])){
    if(is.na(text_SACSMA10[1,j])){
      text_meanBestAllMod[i,j]<- NA
    } else if (text_SACSMA10[1,j]!=text_meanBestAllMod[i,j]){
      text_meanBestAllMod[i,j]<- NA
    } 
  }
}

save(text_meanBestAllMod,file="./Ranking_Data/text_meanBestAllMod")
