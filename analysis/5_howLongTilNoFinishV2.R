...
for(node in 1:(n_nodes-1)){
  ...
  for(i in 1:(length(D))){
    ...
    load(paste0(folder,"/Kriging/seed",seed,"/T_model_Kriging"))
    ...
    load(paste0(folder,"/AKMCS/seed",seed,"/T_model_AKMCS"))
    ...
    if(file.exists(paste0(folder,"/BASS_mmESS/seed",seed,"/S_BASS_list"))){
      IncTime_BASS[i,node+1] <- sum(T_LHS_BASS) + sum(T_BASS)+ sum(T_pred_BASS) + sum(T_model_BASS) + sum(T_BASSSobol) + sum(T_check_BASS) 
    } else IncTime_BASS[i,node+1] <- sum(T_LHS_BASS) + sum(T_BASS)+ sum(T_pred_BASS) + sum(T_model_BASS)
    
    if(file.exists(paste0(folder,"/Kriging/seed",seed,"/Sobol_Kriging_convergesize"))){
      IncTime_Kriging[i,node+1] <- sum(T_LHS_Kriging) + sum(T_Kriging) + sum(T_pred_Kriging) + sum(T_model_Kriging) + sum(T_KrigingSobol) + sum(T_check_Kriging)
    } else IncTime_Kriging[i,node+1] <- sum(T_LHS_Kriging) + sum(T_Kriging) + sum(T_pred_Kriging) + sum(T_model_Kriging)
    
    if(file.exists(paste0(folder,"/AKMCS/seed",seed,"/Sobol_AKMCS_convergesize"))){
      IncTime_AKMCS[i,node+1] <- sum(T_AKMCS) + sum(T_pred_AKMCS) + sum(T_model_AKMCS) + sum(T_AKMCSSobol) + sum(T_check_AKMCS)
    } else IncTime_AKMCS[i,node+1] <- sum(T_AKMCS) + sum(T_pred_AKMCS) + sum(T_model_AKMCS)
...
print("20D")
(IncTime_BASS[5,1:4])/(60^2)
(IncTime_Kriging[5,1:4])/(60^2)
(IncTime_AKMCS[5,1:4])/(60^2)

print("30D")
(IncTime_BASS[6,1:4])/(60^2)
(IncTime_Kriging[6,1:4])/(60^2)
