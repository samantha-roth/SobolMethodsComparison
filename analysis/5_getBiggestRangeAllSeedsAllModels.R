...
#saying which approach is most variable for each number of inputs and run time
...
Mat_BiggestRange <- matrix(NA,nrow=length(tested_D),ncol=length(tested_eval_time))
textMat_BiggestRange <- matrix(NA,nrow=length(tested_D),ncol=length(tested_eval_time))
for (i in 1:(length(tested_D)-1)){
  for (j in 1:length(tested_eval_time)){
    
    Mat_BiggestRange[i,j] <- max(Diff_Time_AKMCS[i,j],Diff_Time_BASS[i,j],
                             Diff_Time_Kriging[i,j],Diff_Time_Sobol[i,j],na.rm=TRUE)
    
    if (Mat_BiggestRange[i,j]==Diff_Time_AKMCS[i,j]) textMat_BiggestRange[i,j] <- "AKMCS"
    
    if (Mat_BiggestRange[i,j]==Diff_Time_BASS[i,j]) textMat_BiggestRange[i,j] <- "BASS"
    
    if (Mat_BiggestRange[i,j]==Diff_Time_Kriging[i,j]) textMat_BiggestRange[i,j] <- "Kriging"
    
    if (Mat_BiggestRange[i,j]==Diff_Time_Sobol[i,j]) textMat_BiggestRange[i,j] <- "Sobol"
...
print(textMat_BiggestRange)

save(Mat_BiggestRange,file="./Ranking_Data/Mat_BiggestRange")
save(textMat_BiggestRange,file="./Ranking_Data/textMat_BiggestRange")