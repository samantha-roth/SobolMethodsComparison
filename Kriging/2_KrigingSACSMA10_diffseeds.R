...
print("2_KrigingSACSMA10_diffseeds.R")
...
foreach(node = 1:4)%dopar%{ 
  ...
  folder <- paste0(folderpath,"SacSma10/Kriging/seed",seed) 
  ...
  while (1>0){
    ...
    X_GP<- Mapping(X_01,Range)
    X_GP<- cbind(X_GP[,1:2],fix_vals[1],X_GP[,3:5],fix_vals[2],X_GP[,6:9],fix_vals[3],X_GP[,10],
    ...
    std <- sqrt(a$MSE)
    print(paste0("max(U) = ",max(std),", range = ",(max(a$Y_hat)-min(a$Y_hat))/20))
