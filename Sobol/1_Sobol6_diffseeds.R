...
foreach(node = 1:4)%dopar%{ 
  ...
  folder<-paste0(folderpath,d,"D/Sobol") #set folder depending on both d and node
...
for(k in 1:6){
  for(node in 1:4){
    seed<- node*k
    d<- D[k]
    folder<-paste0(folderpath,d,"D/Sobol/seed",seed)
    load(paste0(folder,"/Sobol_convergesize"))
    print(paste0("node: ", node,", k: ",k,", Sobol_convergesize: ", Sobol_convergesize))
  }
}